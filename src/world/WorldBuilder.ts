...
export class WorldBuilder {
  ...
  /**
   * FAZ A PECA SUMIR QUANDO A DUPLA ANDA ATRAS DELA.
   *
   * Este e o conserto do unico defeito estrutural da camera isometrica fixa:
   * predio alto tem um PONTO CEGO atras de si. O restaurante do clube tem 5,5
   * de altura, e a 34° ele engole quase 4 m da faixa de tras — quem anda ali
   * some da tela, e some junto com a porta que estiver naquela parede.
   *
   * A conta e a mesma de sempre, so que feita todo quadro: caminha da posicao
   * da pessoa NA DIRECAO DA CAMERA e pergunta, a cada passo, se a peca esta ali
   * e se ela e alta o bastante para cobrir a linha de visao naquele ponto. A
   * direcao vem do giro atual (`anguloDaCamera`), e nao de uma constante: o
   * jogador gira a camera de 45 em 45, e ao girar 180° o predio deixa de tapar
   * — a peca tem que voltar a ser solida sozinha.
   *
   * OS MATERIAIS SAO CLONADOS. O `toon()` cacheia por cor: mexer na opacidade
   * do material compartilhado apagaria todas as outras pecas da mesma cor no
   * cenario inteiro. O clone custa alguns materiais a mais para UM predio.
   */
  transparenteQuandoAtras(
    peca: THREE.Object3D,
    caixa: { x: number; z: number; largura: number; profundidade: number; altura: number },
    minimo = 0.26,
  ): void {
    ...
    peca.traverse((o) => {
      ...
      if (!m.isMesh) return;
      const clone = Array.isArray(m.material)
        ? m.material.map((mat) => mat.clone())
        : m.material.clone();
      m.material = clone;
      for (const mat of Array.isArray(clone) ? clone : [clone]) materiais.push(mat);
    });

    /** o ponto esta dentro da planta da peca? */
    const dentro = (x: number, z: number): boolean =>
      Math.abs(x - caixa.x) < caixa.largura / 2 &&
      Math.abs(z - caixa.z) < caixa.profundidade / 2;

    /** a peca tapa quem esta neste ponto, olhando com a camera de agora? */
    const tapado = (px: number, pz: number, giro: number, tan: number): boolean => {
      const dx = Math.sin(giro);
      const dz = Math.cos(giro);
      // a cabeca de quem esta em pe fica a 1,7; o alcance maximo e a altura da
      // peca dividida pela tangente, que e onde a linha de visao passa por cima
      const alcance = Math.max(0, (caixa.altura - 1.7) / tan);
      for (let t = 0.5; t <= alcance; t += 0.5) {
        if (dentro(px + dx * t, pz + dz * t)) return true;
      }
      return false;
    };
    ...
    let alfa = 1;
    this.onUpdate((dt) => {
      const { giro, inclinacao } = this.game.anguloDaCamera();
      const tan = Math.tan(inclinacao);
      const eu = this.game.playerPosition();
      const ele = this.game.companionPosition();
      const esconde = tapado(eu.x, eu.z, giro, tan) || tapado(ele.x, ele.z, giro, tan);

      const alvo = esconde ? minimo : 1;
      alfa += (alvo - alfa) * Math.min(1, dt * 6);
      const solido = alfa > 0.99;
      for (const mat of materiais) {
        if (mat.transparent === solido) {
          mat.transparent = !solido;
          mat.depthWrite = solido;
