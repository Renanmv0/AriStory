...
/**
 * 4: PEÇA COMPRADA VIRA ESTOQUE DO GUARDA-ROUPA.
 *
 * Este bloco existe por um acidente de verdade: o Renan comprou um vestido de
 * R$ 96, vestiu, descartou sem querer e teve que comprar de novo. A regra que
 * o conserta NÃO é proibir o descarte — é a compra virar estoque, como as
 * peças do armário do Ari: descartar tira do corpo, e o armário repõe na
 * próxima abertura. Então são três medidas: a tela OFERECE o descarte, o
 * descarte ACONTECE, e a peça VOLTA.
 */
...
await page.keyboard.press('KeyI');
await page.waitForTimeout(500);
// e agora o que prova a regra nova: abrir o armário traz a peça de volta
await page.evaluate(() => window.jogo.abrirGuardaRoupa());
await page.waitForTimeout(1100);
const reposto = await page.evaluate(() => ({
  acervo: window.jogo.wardrobeItems().map((x) => x.id),
  doParceiro: window.jogo.wardrobeItems(window.jogo.companionId()).map((x) => x.id),
  compradas: [...window.jogo.save.compradas],
}));
await page.keyboard.press('Escape');
await page.waitForTimeout(500);
/*
 * A cena volta como estava de graça: o descarte tirou a peça do corpo e a
 * reposição do armário devolveu ela ao acervo — que é onde a checagem do
 * espelho, lá na frente, procura por ela.
 */
const despiu = reposto.acervo;
...
if (!descarteOferecido) falhas.push('a tela nao oferece descartar uma peca comprada');
if (foiDescartada.vestindo.includes(foiDescartada.id)) {
  falhas.push(`o descarte da peca comprada foi recusado: ${JSON.stringify(foiDescartada)}`);
}
if (foiDescartada.acervo.includes(foiDescartada.id)) {
  falhas.push(`a peca descartada continuou no acervo sem reabrir o armario`);
}
if (!reposto.acervo.includes(compra.acervo[0])) {
  falhas.push(`o armario nao repos a peca COMPRADA: ${JSON.stringify(reposto)}`);
}
if (!reposto.doParceiro.includes(compra.acervo[0])) {
  falhas.push(`a peca comprada nao chegou ao parceiro: ${JSON.stringify(reposto.doParceiro)}`);
}
if (!reposto.compradas.includes(compra.acervo[0])) {
  falhas.push(`a compra nao ficou anotada no save: ${JSON.stringify(reposto.compradas)}`);
}
if (!despiu.includes(compra.acervo[0])) {
  falhas.push(`a peca nao voltou ao acervo antes do espelho: ${JSON.stringify(despiu)}`);
}
...
console.log('    descarte oferecido:', descarteOferecido, '(tem que ser 1) · descartou',
  foiDescartada.id, '· o armario repos:', JSON.stringify(reposto.acervo),
  '· anotadas:', JSON.stringify(reposto.compradas));
